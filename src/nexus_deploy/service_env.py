...
def _render_cloudbeaver(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    return RenderedEnv(
        env_vars={
            "CLOUDBEAVER_SERVER_NAME": "Nexus CloudBeaver",
            "CLOUDBEAVER_ADMIN_NAME": "nexus-cloudbeaver",
            "CLOUDBEAVER_ADMIN_USER": "nexus-cloudbeaver",
            "CLOUDBEAVER_ADMIN_PASSWORD": c.cloudbeaver_admin_password or "",
        },
    )
...
def _render_pg_ducklake(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    """Two-path: if all four Hetzner S3 vars are set, generate the
    bootstrap SQL with a ``CREATE SECRET`` for the pg-ducklake S3
    backend; otherwise emit a local-only fallback that just unsets
    any pre-existing secret. Mirrors deploy.sh:484-538.
    ...
    if has_s3:
        ...
        bucket_url = f"s3://{c.hetzner_s3_bucket_pgducklake}/"
        sql = f"""-- Auto-generated by nexus_deploy.service_env (was deploy.sh)
-- pg-ducklake S3 bootstrap for Hetzner Object Storage.
DROP SECRET IF EXISTS hetzner_s3_secret;
CREATE SECRET hetzner_s3_secret (
    TYPE S3,
    KEY_ID '{_escape_sql(c.hetzner_s3_access_key or "")}',
    SECRET '{_escape_sql(c.hetzner_s3_secret_key or "")}',
    REGION '{_escape_sql(c.hetzner_s3_region or "auto")}',
    ENDPOINT '{_escape_sql(endpoint)}',
    URL_STYLE 'path'
);
ATTACH 'ducklake:postgres:dbname=ducklake host=pg-ducklake user=nexus-pg-ducklake password={_escape_sql(c.pgducklake_password or "")}' AS ducklake (
    DATA_PATH '{_escape_sql(bucket_url)}'
);
"""
    else:
        sql = """-- Auto-generated by nexus_deploy.service_env (was deploy.sh)
-- pg-ducklake local-only bootstrap (no Hetzner S3 configured).
DROP SECRET IF EXISTS hetzner_s3_secret;
"""
    return RenderedEnv(
...
def _render_pgadmin(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    return RenderedEnv(
        env_vars={
            "PGADMIN_DEFAULT_EMAIL": e.admin_email or "",
            "PGADMIN_DEFAULT_PASSWORD": c.pgadmin_password or "",
        },
    )
...
def _render_prefect(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    return RenderedEnv(
        env_vars={
            ...
            "PREFECT_API_URL": f"https://prefect.{e.domain or ''}/api",
        },
    )
...
def _render_rustfs(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    return RenderedEnv(
        env_vars={
            "RUSTFS_ROOT_USER": "nexus-rustfs",
            "RUSTFS_ROOT_PASSWORD": c.rustfs_root_password or "",
        },
    )
...
def _render_garage(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    full configuration. Mirrors deploy.sh:674-705."""
...
def _render_filestash(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    base64. Mirrors deploy.sh:750-855.

    Special handling:
    - If admin password is set, run ``htpasswd -nbBC 10`` to generate
      bcrypt hash. Escape ``$`` → ``$$`` for docker-compose env parsing.
    - If any S3 backend (R2 / Hetzner / External) is configured,
      build a CONFIG_JSON dict with the connections + middleware
      params, then base64-encode (newlines stripped).
    """
    domain = e.domain or ""
    admin_hash = ""
    ...
    has_r2 = bool(c.r2_data_endpoint and c.r2_data_access_key and c.r2_data_secret_key)
    has_hetzner = bool(c.hetzner_s3_server and c.hetzner_s3_access_key and c.hetzner_s3_secret_key)
    has_external = bool(
        c.external_s3_endpoint and c.external_s3_access_key and c.external_s3_secret_key,
    )
            ...
            connections.append(
                {
                    ...
                    "region": c.hetzner_s3_region or "auto",
                    "path": c.hetzner_s3_bucket_general or "",
                },
            )
        if has_external:
            connections.append(
                {
                    "label": c.external_s3_label or "External Storage",
                    "type": "s3",
                    "endpoint": c.external_s3_endpoint or "",
                    "access_key_id": c.external_s3_access_key or "",
                    "secret_access_key": c.external_s3_secret_key or "",
                    "region": c.external_s3_region or "auto",
                    "path": c.external_s3_bucket or "",
                },
            )
...
def _render_woodpecker(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    deploy.sh:858-878. Gitea OAuth client+secret are populated by
    deploy.sh AFTER orchestrator.run-all (via separate ``cat >>``
    block — see deploy.sh:2640-2656). This render only emits the
    fields the legacy main block produced; the OAuth append is NOT
    part of this module."""
    ...
    return RenderedEnv(
        env_vars={
            ...
            "WOODPECKER_ADMIN_USER": c.admin_username or "admin",
            "WOODPECKER_HOST": f"https://woodpecker.{e.domain or ''}",
            "WOODPECKER_GITEA_URL": f"https://gitea.{e.domain or ''}",
            "DOMAIN": e.domain or "",
        },
    )
...
def _render_flink(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    return RenderedEnv(
        env_vars={
            ...
            "HETZNER_S3_SECRET_KEY": c.hetzner_s3_secret_key or "",
            "HETZNER_S3_BUCKET": c.hetzner_s3_bucket_general or "",
            "FLINK_TASK_MANAGER_NUM_TASK_SLOTS": "2",
        },
    )


def _render_dinky(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    """Dinky: admin password optional. If empty, render is still
    valid (dinky has its own internal default), but operator gets
    a warning via the spec result detail."""
    if _empty(c.dinky_admin_password):
        return RenderedEnv(
            env_vars={"DINKY_ADMIN_PASSWORD": ""},
            skip_reason=None,  # not skipped, just warned
        )
...
def _render_nocodb(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    return RenderedEnv(
        env_vars={
            ...
            "NOCODB_ADMIN_EMAIL": e.admin_email or "",
            "NOCODB_ADMIN_PASSWORD": c.nocodb_admin_password or "",
            "NOCODB_JWT_SECRET": c.nocodb_jwt_secret or "",
            "DOMAIN": e.domain or "",
        },
    )
