...
EOF
    ...
    if [ -n "$HETZNER_S3_BUCKET_PGDUCKLAKE" ] && [ -n "$HETZNER_S3_ACCESS_KEY" ]; then
...
SELECT duckdb.create_simple_secret(
    type := 'S3',
    name := 'ducklake_s3',
    key_id := '${HETZNER_S3_ACCESS_KEY}',
    secret := '${HETZNER_S3_SECRET_KEY}',
    region := '${HETZNER_S3_REGION}',
    endpoint := '${HETZNER_S3_SERVER}',
    url_style := 'path',
    scope := 's3://${HETZNER_S3_BUCKET_PGDUCKLAKE}/'
);

-- Set default storage path for new DuckLake tables
ALTER SYSTEM SET ducklake.default_table_path = 's3://${HETZNER_S3_BUCKET_PGDUCKLAKE}/';
