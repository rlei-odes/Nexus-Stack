...
Sample Kestra flows, notebooks, dbt models, and other starter material that ships with Nexus-Stack lives under [`examples/workspace-seeds/`](../examples/workspace-seeds/) and gets auto-seeded into every student's Gitea workspace repo on spin-up. Conventions, the path-mapping rules, and the **no-schedule-triggers in seeded flows** rule are documented in [`examples/README.md`](../examples/README.md). Read that before adding a new sample.
