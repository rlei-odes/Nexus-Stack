...
| **[SFTPGo](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/sftpgo.md)** | `drakkan/sftpgo:v2.7.1` | SFTP/SCP/WebDAV server with a web admin UI. Configured here with the R2 datalake bucket as its S3 backend, so external tools that only speak SFTP/WebDAV can read and write the same files the rest of the stack accesses via S3. |
